...
class ChaptersListController: UITableViewController {
    ...
    lazy var spinner: UIActivityIndicatorView = {
        ...
        let spinner = UIActivityIndicatorView(frame: CGRect(x: screenWidth/2, y: screenHeight/2, width: 50, height: 50))
        spinner.center = CGPoint(x: screenWidth/2, y: screenHeight/2)
    ...
    @objc private func downloadedAlready() {
        ...
        let alert = UIAlertController(title: "Whoops!",
                                      message: "Здається, ви вже маєте копію усіх розділів цієї манги на своєму телефоні",
    ...
    private func setupHeader() {
        ...
        if let cdmodel = cdmodel {
            let model = MangaDetailModel(title: cdmodel.title!,
                                         coverURL: cdmodel.cover!,
                                         chaptersCount: Int(cdmodel.chaptersCount),
                                         volumesCount: Int(cdmodel.volumesCount),
                                         genras: cdmodel.genras!,
                                         description: cdmodel.descript!,
                                         directory: cdmodel.directory!,
                                         chapters: nil,
                                         author: cdmodel.author!,
                                         status: cdmodel.status!,
                                         translation: cdmodel.translation!,
                                         release: Int(cdmodel.releaseYear),
                                         isTranslated: cdmodel.isTranslated,
                                         isFinished: cdmodel.isFinished)
...
extension ChaptersListController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let gridVC = GridViewController(chapters: chapters, title: title, index: index)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: ChaptersListTableViewCell.indentifier, for: indexPath) as? ChaptersListTableViewCell else {
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let swipeAction = UIContextualAction(style: .normal, title: "") { _, _, complition in
        ...
        let swiperConfig = DataManager.shared
            .existsChapter(with: chapterToDownload.chapterID) ? UISwipeActionsConfiguration() : UISwipeActionsConfiguration(actions: [swipeAction])
...
extension ChaptersListController: ChaptersHeaderViewDelegate {
    func downloadThisManga() {
        ...
        let alert = UIAlertController(title: "Ви дійсно хочете завантажити всі розділи?",
                                      message: "Це може займати багато пам'яті вашого телефону",
                                      preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "Завантажити",
                                      style: .default,
                                      handler: { _ in
    ...
    func readThisManga() {
        let title = (cdmodel?.title != nil ? cdmodel?.title : model?.title)!
        let gridVC = GridViewController(chapters: chapters, title: title)
        navigationController?.pushViewController(gridVC, animated: true)
    }
