...
class DowloadsTableViewCell: UITableViewCell {
    ...
    override func layoutSubviews() {
        ...
        titleLabel.frame = CGRect(x: thumbnailImageView.right+5,
                                  y: 5,
                                  width: contentView.width-thumbnailImageView.width-20,
                                  height: thumbnailImageView.height/2)
        titleLabel.sizeToFit()
        titleLabel.frame.size.width = contentView.width-thumbnailImageView.width-20
        chaptersCountLabel.frame = CGRect(x: titleLabel.left,
                                          y: titleLabel.bottom+5,
                                          width: titleLabel.width,
                                          height: 20)
        genrasLabel.frame = CGRect(x: chaptersCountLabel.left,
                                   y: chaptersCountLabel.bottom+5,
                                   width: titleLabel.width,
                                   height: thumbnailImageView.height-titleLabel.height-chaptersCountLabel.height-10)
        separatorView.frame = CGRect(x: 20,
                                     y: contentView.height-1,
                                     width: contentView.width-40,
                                     height: 1)
    }

    public func configure(with model: Manga) {
        SDImageCache.shared.queryImage(forKey: model.cover, options: .continueInBackground, context: nil, cacheType: .all) { image, _, _  in
