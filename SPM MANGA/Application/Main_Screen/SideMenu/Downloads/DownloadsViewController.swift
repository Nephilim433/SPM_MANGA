...
class DownloadsViewController: UIViewController {
    ...
    private func showDummyLabel() {
        dummyLabel.isHidden = !mangas.isEmpty
    }
...
extension DownloadsViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let chaptersVC = ChaptersListController(model: mangas[indexPath.row])
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: DowloadsTableViewCell.identifier, for: indexPath) as? DowloadsTableViewCell else {
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let swipeAction = UIContextualAction(style: .destructive, title: "Видалити") { _, _, done in
